...
drawn). Inputs: one scene's narration, its chosen shape, and the video's design brief.
...
- **Narration** — exactly what the viewer hears during this scene.
...
- **Nodes: between N and M** — a hard range. A diagram outside it is rejected and you are asked
  again, which wastes a turn. Stay inside it.
...
- `nodes` — the things on the board. Each has an `id` you invent, a `label`, and optionally:
  ...
  - `emphasis` — the one focal point. **At most one node in the whole diagram.**
  - `anchor` — see *Timing*.
...
- **One focal point**, or none. Two emphasised nodes is no focal point at all.
...
- `anchor` must be a **verbatim substring of this scene's narration**. Copy it exactly — do not
  paraphrase, re-case, or repunctuate it. A phrase that does not match is silently ignored and the
  element inherits the previous one's timing, which looks like a sync bug.
